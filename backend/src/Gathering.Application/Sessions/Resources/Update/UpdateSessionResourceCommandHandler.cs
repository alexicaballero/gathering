...
public sealed class UpdateSessionResourceCommandHandler : ICommandHandler<UpdateSessionResourceCommand>
{
  ...
  public async Task<Result> HandleAsync(UpdateSessionResourceCommand request, CancellationToken cancellationToken = default)
  {
    var validationResult = await _validator.ValidateAsync(request, cancellationToken);
    if (!validationResult.IsValid)
    {
      var errorMessages = string.Join("; ", validationResult.Errors.Select(e => e.ErrorMessage));
      return Result.Failure(Error.Validation("UpdateSessionResource.ValidationFailed", errorMessages));
    }
